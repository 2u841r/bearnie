...
const codeDisplay = document.getElementById('code-display') as HTMLElement | null;
const editorContainer = document.getElementById('code-editor-container') as HTMLElement | null;
const loadingIndicator = document.getElementById('code-image-loading') as HTMLElement | null;
const imageBackground = document.getElementById('code-image-background') as HTMLElement | null;
const imageCard = document.getElementById('code-image-card') as HTMLElement | null;
const themeSelect = document.getElementById('theme-select') as HTMLSelectElement | null;
const darkmodeToggle = document.getElementById('darkmode-toggle') as HTMLElement | null;
const linenumbersToggle = document.getElementById('linenumbers-toggle') as HTMLElement | null;
...
async function initHighlighter() {
  try {
    ...
    const customThemeFiles = [
      ...
      { file: 'serendipity-midnight-electric', name: 'Serendipity Midnight Electric' },
...
function updateHighlighting() {
  ...
  const lang = state.language === 'auto' ? state.detectedLanguage : state.language;
  ...
  try {
    ...
    tokens.tokens.forEach((line: any[], i: number) => {
      ...
      if (state.showLineNumbers) {
        ...
        codeHtml += `<span style="color: ${theme.fg}33; user-select: none; display: inline-block; width: ${gutterWidth}ch; margin-right: 1.5em; text-align: right; line-height: 1.7;">${num}</span>`;
      }
      // Tokens
      if (line.length === 0) {
        codeHtml += '\n';
    ...
    codeDisplay.innerHTML = `<pre style="margin:0;padding:0;background:transparent;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;max-width:100%;width:100%;box-sizing:border-box;line-height:1.7;"><code style="color:${t.fg};display:block;max-width:100%;width:100%;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;box-sizing:border-box;line-height:1.7;">${escapeHtml(state.code || ' ')}</code></pre>`;
  }
}
...
function applyBackground() {
  ...
  const adjustColor = (hex: string, percent: number) => {
    ...
    const adjust = (val: number) => Math.max(0, Math.min(255, val + 255 * percent));
  ...
  const bgColor = isDark ? adjustColor(baseColor, 0.02) : adjustColor(baseColor, -0.01);
  ...
  let patternEl = imageBackground.querySelector('.pattern-overlay') as HTMLElement | null;
  if (!patternEl) {
...
const exportArea = document.getElementById('code-image-export-area') as HTMLElement | null;
...
function prepareForExport() {
  const textarea = document.getElementById('code-input') as HTMLTextAreaElement | null;
...
function initExportListeners() {
  ...
  document.getElementById('export-copy-image')?.addEventListener('click', copyImage);
  document.getElementById('export-copy-url')?.addEventListener('click', copyUrl);
